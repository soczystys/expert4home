...
Tablica kanban przechowuje User Stories z następującym przepływem: TODO (funkcjonalność, która jest dopiero planowana) $\rightarrow$ In progress (w trakcie wykonywania) $\rightarrow$ Resolved (problem rozwiązany) $\rightarrow$ Closed (rozwiązanie potwierdzone i zamknięte)
 
Pełna lista User Stories znajduje się w backlogu.

Zadania (Tasks) są wykonywane w ramach sprintów. Są one podzielone na User Stories, które z kolei są rozdzielone na konkretne zadania. Te z kolei mają następujący przepływ: Open $\rightarrow$ In Progress $\rightarrow$ Code Review $\rightarrow$ Merge (testy całości po merge'u) $\rightarrow$ Closed.
